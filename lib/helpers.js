...
function insertProduct(values) {
    const sql = 'INSERT INTO product(categorylevel2_id, categorylevel3_id, name, description, status, made, sku)'
    + 'VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING id';

    return checkName('product', [values[2].trim()])
    .then(data => {
        if (!data) {
            return db.excuteQuery(sql, values)
            .then(res => {
...
function insertProductVariant(values) {
    const sql = 'INSERT INTO productvariant(product_id, name, sku, price, stockamount) VALUES($1, $2, $3, $4, $5) RETURNING id';
